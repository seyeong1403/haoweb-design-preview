    ...
    function tick() {
      secs.forEach(function (sec) {
        ...
        if (frame && maskText) {
          ...
          /* ⭐ 2026-08-17 세영 지적 — "HAO 가 오른쪽으로 치우쳐 있다"(레퍼런스 mm 은 정중앙).
             원인은 이 OX 를 **처음부터 끝까지 190 으로 고정**한 것이었다. 화면 중앙에 오는
             글자 좌표가 px = -OX 라, 글자가 작아 '단어'로 읽히는 앞 구간에서도 통째로
             오른쪽으로 밀려 있었다(sc 0.26 일 때 약 +49px, 커질수록 더).
             ⚠ 그렇다고 0 으로 두면 안 된다 — 위 주석대로 확대되면 'A' 의 삼각 구멍이
               화면 중앙을 덮어 사진이 사라진다.
             → **앞에서는 0(정중앙), 커지면서 190(H 세로획)** 으로 옮긴다.
               p 0.08 까지는 단어로 읽히니 정중앙, 0.20 이면 글자가 이미 화면보다 커진다
               (p 0.114 에서 sc 3.53 = 글자 높이 840px). 그 사이에서 기준점을 넘긴다. */
          var OX = 0.95 * BASE * clamp((p - 0.08) / 0.12, 0, 1);
